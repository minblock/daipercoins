...
const char * const BITCOIN_CONF_FILENAME = "daipercoins.conf";
...
#else
    const char* pszModule = "daipercoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Daipercoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Daipercoins";
#else
    // Unix
    return pathRet / ".daipercoins";
#endif
#endif
}
